...
public class Main {
    ...
    public static Object[] removeDuplicatesFromArray(int[] array) {
        LinkedList <Integer> RDFA = new LinkedList<>();
        for (int arr: array){
            if (RDFA.contains(arr)){
                continue;
            }
            RDFA.add(arr);
    ...
    public static int sumOfTwoLargest(int[] array) {
        ...
        if (array.length==0)
            ...
            int largest0 = Math.min(array[0], array[1]);
            int largest1 = Math.max(array[0], array[1]);
            ...
            //I thought of doing this with recursion, but I did not
            //return sumOfTwoLargest()+sumOfTwoLargest()
